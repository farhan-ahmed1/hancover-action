 ...
 */
export async function parseJaCoCoFile(filePath: string, options?: StreamingParseOptions): Promise<ProjectCov> {
    try {
        // Get file stats for progress reporting and timeout calculation
        const stats = await import('fs/promises').then(fs => fs.stat(filePath));
        const fileSizeBytes = stats.size;
        
        core.info(`Processing JaCoCo file: ${filePath} (${formatBytes(fileSizeBytes)})`);
